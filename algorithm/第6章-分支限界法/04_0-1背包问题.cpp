 ...
 *
 * 【阅读指引 / 易错点】
 *   - 节点信息：通常包含层号 i、当前重量/价值、以及按分数背包计算的价值上界 bound。
 *   - 活节点表策略：用优先队列按 bound 从大到小取出最有希望的节点扩展（最大化问题）。
 *   - 易错点：bound 计算要在容量不足时按“可拆分”取一部分，这只是上界估计，不是可行解。
 *
 * 【实现提示】
 *   - 活节点表：按 bound 最大优先扩展，更快得到较好的 bestValue。
 *   - 界函数：CalcBound 必须是上界（乐观估计），不能低估。
