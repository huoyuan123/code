 ...
 *
 * 【阅读指引 / 易错点】
 *   - 活节点表：这里用“优先队列（最小堆）”组织活节点，每次扩展当前距离最小的节点（最佳优先）。
 *   - 本质联系：这就是 Dijkstra 的分支限界表述；界函数可理解为当前 dist（越小越优先扩展）。
 *   - 易错点：优先队列里会存在过时条目，需要在出队时用 dist[u] 校验并跳过。
 *
 * 【实现提示】
 *   - 活节点扩展：弹出当前最小 dist 节点，对其邻接边做松弛。
 *   - 界函数：当前 dist 即下界，按最小优先队列扩展。
 *   - 状态一致性：出队时若距离已过时要跳过，避免重复扩展。
 *
 * 算法来源：《计算机算法设计与分析(第5版)》第6.2节

 *
 * 解空间树：子集树（Subset Tree）
 * 递归方式：广度优先搜索(BFS)（最佳优先）
