 ...
 *
 * 【阅读指引 / 易错点】
 *   - 状态 m[i][j]：表示从 A_i 到 A_j 连乘的最少标量乘次数；s[i][j] 记录最优断开点用于还原括号。
 *   - 维度数组 p：长度为 n+1，其中 A_i 的维度是 p[i-1]×p[i]；最常见错误是把 p 下标写偏。
 *   - 遍历顺序：按链长 r 从小到大填表，保证用到的子问题 m[i][k]、m[k+1][j] 已经计算完。
 */
#include <iostream>
#include <vector>
#include <climits>
