 ...
 *
 * 【阅读指引 / 易错点】
 *   - Partition 是核心：它完成“把 pivot 放到最终位置，并保证左边 ≤ pivot、右边 ≥ pivot”。
 *   - 最坏情况：当 pivot 选择很差（如已排序数组还选首元素）时会退化到 O(n^2)；随机化 pivot 可显著降低退化概率。
 *   - 递归区间：一定要用 pivotPos-1 与 pivotPos+1，避免死递归；并注意闭区间边界。
 *
